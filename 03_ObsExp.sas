/*Calcul des ratios Observed/expected*/
...
/*Définition d'une macro de calcul des ratios Observed/Expected 
...
pour les vaccinations des tables "allvac_x" (x : duree de FU post-vaccinal=delai)
selon type de schema vaccinal (regimen)*/



%macro OBSEXP(regimen=,diag=,delai=,DP_only=);

/*On filtre la table des vaccins (allvac_42 ou allvac_7) selon "regimen"
(parmi vaccins durant PP (LMP-70/EOPE) et <23/12/23) */
PROC SQL;
...
/*Ajout de TMP1: vaccins et periodes de FU post vaccinal (calculées dans script table_vac)
...
%macro OBSEXP_list(regimen=,diag_list=,delai=,DP_only=);
    %let num_diags = %sysfunc(countw(&diag_list));
    %do i = 1 %to &num_diags;
        %let diag = %scan(&diag_list, &i);       
        /* Appliquer macro OBSEXP sur chaque diag de la liste*/
        %OBSEXP(regimen=&regimen.,diag=&diag.,delai=&delai.,DP_only=&DP_only.);
  
    %end;
%mend;

/*TEST*/
%OBSEXP_list(regimen=all_BNT,delai=42,DP_only=0,
...
/*Resultats*/
data rep.OE_alldiag_BNT42_all ;
		set WORK.ALL_:;
	run;

/*Donnees intermediaires*/
data rep.OE_alldiag_BNT42 ;
		set WORK.OE_:;
	run;
